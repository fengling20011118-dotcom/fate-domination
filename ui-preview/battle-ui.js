 ...
 const api={root};
    ...
    function renderDiscardView(){
      ['situation','event'].forEach(kind=>{
        const cards=publicDiscards[kind],pile=root.querySelector('.'+kind+'-discard'),list=root.getElementById(kind+'-discard-list'),total=root.getElementById(kind+'-discard-total');
        ...
        pile.querySelector('.discard-count').textContent=String(cards.length);
        if(cards.length){const top=cards[cards.length-1],img=pile.querySelector('img');img.src=top.image;img.alt=top.name+' · 弃牌堆顶牌'}
      });
      const summary=root.querySelector('.discard-summary'),total=publicDiscards.situation.length+publicDiscards.event.length;
    ...
    function commitRoundDiscards(){
      ...
      const targets={situation:root.querySelector('.situation-discard'),event:root.querySelector('.event-discard')};
      ...
      return Promise.all(cards.map((card,index)=>animateResolvedDiscard(card,targets[card.kind],index*150))).then(()=>{
        Object.values(targets).forEach(target=>{if(!target)return;target.style.removeProperty('opacity');target.classList.remove('landing');void target.offsetWidth;target.classList.add('landing')});
