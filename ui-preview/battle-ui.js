...
function render(host,options={}){
 ...
 root.innerHTML='<style>'+css+'</style>'+markup;
  root.querySelector('style').textContent+=`
    ...
    .discard-modal{position:fixed;z-index:120;inset:0;display:none;place-items:center;padding:28px;background:radial-gradient(circle at 50% 44%,rgba(30,34,46,.34),rgba(2,3,7,.9) 58%);backdrop-filter:blur(7px)}
    ...
    .top-tools{top:17px;align-items:center}.top-tools .turn-clock{height:46px;display:flex;align-items:stretch;overflow:hidden;border:1px solid rgba(214,174,82,.36);border-radius:6px;background:linear-gradient(180deg,rgba(25,29,38,.98),rgba(10,13,19,.98));box-shadow:0 6px 16px rgba(0,0,0,.34)}
    .turn-clock>span{min-width:74px;display:grid;grid-template-columns:auto auto;align-items:center;gap:6px;padding:0 9px;border-right:1px solid rgba(255,255,255,.08)}.turn-clock>span:last-child{border-right:0}.turn-clock small{color:#8c95a3;font-size:9px}.turn-clock strong{color:#f3e4b4;font:700 15px/1 Georgia,serif;letter-spacing:.04em}.turn-clock.overtime{border-color:rgba(198,74,81,.72);box-shadow:0 0 16px rgba(189,75,82,.18)}.turn-clock.overtime #round-time{color:#ff9da3}.turn-clock.bank-empty #bank-time{color:#ff737b}.top-tools>button{height:38px}
    .turn-order{right:510px}.power-meter{cursor:help}.power-meter.projected{border-color:rgba(100,184,226,.72);box-shadow:0 0 12px rgba(92,169,214,.15)}.power-meter.projected strong{color:#aee0ff}
    .skill-confirm-panel{width:min(560px,92vw)!important;height:auto!important;min-height:0!important}.skill-confirm-notice{margin:18px;padding:13px 15px;border-left:3px solid var(--gold);color:#b9c1cc;background:rgba(214,174,82,.06);font-size:12px;line-height:1.65}.skill-confirm-panel .choice-actions{margin-top:0}
    ...
    @media(max-width:1700px){.top-tools .turn-clock{height:38px}.turn-clock>span{min-width:62px;padding:0 7px}.turn-clock strong{font-size:13px}.top-tools>button{height:32px}.turn-order{right:auto}}
    @media(max-width:1260px){.turn-clock>span:last-child{display:none}.top-tools{right:116px}.top-tools button{padding:0 8px}}
  `;
 ...
 const api={root};
    ...
    root.addEventListener('pointerover',event=>{
      ...
      if(target.matches('.hand .card')){
        ...
        previewDelayTimer=window.setTimeout(()=>{if(previewHoverTarget===target&&target.matches(':hover'))renderCardPreview(target,previewPointer)},1000);
    ...
    function refreshPlaySelection() {
      const cards=handCards(),selected=cards.filter(card=>card.classList.contains('selected')),played=cards.filter(card=>card.classList.contains('played')),projection=projectedAttackPower([...played,...selected]);
      confirmPlay.textContent='确认出牌 '+selected.length+'/2';
      confirmPlay.disabled=selected.length!==2;
      powerMeter.querySelector('strong').textContent=String(projection.total);
      powerMeter.classList.toggle('projected',selected.length>0);
      powerMeter.dataset.info='预计威力|基础攻击 '+projection.base+'\n局势修正 +'+projection.situation+'\n地利 +'+projection.terrain+'\n事件修正 +'+projection.event+'\n\n当前仅作 UI 预估；接入规则层后以权威战力模拟结果为准。';
    }
    ...
    const turnClock=root.getElementById('turn-clock'),roundTimeNode=root.getElementById('round-time'),bankTimeNode=root.getElementById('bank-time');
    const turnClockState={roundRemaining:90,bankRemaining:1200,overtime:false,autoPlayed:false};
    ...
    function renderTurnClock(){
      if(roundTimeNode)roundTimeNode.textContent=formatClock(turnClockState.roundRemaining);
      if(bankTimeNode)bankTimeNode.textContent=formatClock(turnClockState.bankRemaining);
      turnClock?.classList.toggle('overtime',turnClockState.overtime);
      turnClock?.classList.toggle('bank-empty',turnClockState.bankRemaining<=0);
      if(turnClock)turnClock.title=turnClockState.overtime?'本轮免费时间已用完，正在消耗总时长':'每轮拥有 1 分 30 秒；超时后消耗总时长';
    }
    function autoPlayOnTimeout(){
      if(turnClockState.autoPlayed)return false;
      ...
      toast.textContent=available.length>=2?'计时结束，已自动选择当前基础威力最高的两张牌':'计时结束，已自动结束行动阶段';
    ...
    function tickTurnClock(){
      ...
      if(!previewReady){renderTurnClock();return}
    ...
    const skillConfirmModal=root.getElementById('skill-confirm-modal'),skillConfirmPanel=skillConfirmModal?.querySelector('.skill-confirm-panel'),skillConfirmTitle=root.getElementById('skill-confirm-title'),skillConfirmDesc=root.getElementById('skill-confirm-desc');
    let pendingSkillButton=null;
    function closeSkillConfirm(){closeAnimatedModal(skillConfirmModal,skillConfirmPanel);skillConfirmModal?.setAttribute('aria-hidden','true');pendingSkillButton=null}
    function openSkillConfirm(button,skill){
      pendingSkillButton=button;
      if(skillConfirmTitle)skillConfirmTitle.textContent='确认使用【'+(skill?.name||button.dataset.normalSkill)+'】';
      if(skillConfirmDesc)skillConfirmDesc.textContent=(skill?.type?skill.type+' · ':'')+(skill?.text||'确认后提交技能使用，等待规则层结算。');
      openAnimatedModal(skillConfirmModal,skillConfirmPanel);skillConfirmModal?.setAttribute('aria-hidden','false');
    }
    function executeSkillButton(button){
      if(!button)return;
      const skillName=button.dataset.normalSkill;
      ...
      toast.textContent='【'+skillName+'】已确认，等待目标或规则结算';
      toast.classList.add('show');setTimeout(()=>toast.classList.remove('show'),1700);
    }
    root.getElementById('close-skill-confirm')?.addEventListener('click',closeSkillConfirm);
    root.getElementById('cancel-skill-confirm')?.addEventListener('click',closeSkillConfirm);
    skillConfirmModal?.addEventListener('click',event=>{if(event.target===skillConfirmModal)closeSkillConfirm()});
    root.getElementById('confirm-skill-use')?.addEventListener('click',()=>{const button=pendingSkillButton;closeSkillConfirm();executeSkillButton(button)});
    root.querySelectorAll('[data-normal-skill]').forEach(button=>button.addEventListener('click',()=>{
      const source=button.dataset.skillKind==='master'?masterData:servantData,skill=(source?.skills||[]).find(item=>item.name===button.dataset.normalSkill);
      if(button.dataset.choiceSkill&&skill&&openChoice(skill,button.dataset.skillKind==='master'?'御主能力':'从者技能'))return;
      openSkillConfirm(button,skill);
    }));
    ...
    function openSettlementDemo(){hidePreview();settlementDemo?.classList.remove('victory');settlementDemo?.classList.remove('open');void settlementDemo?.offsetWidth;settlementDemo?.classList.add('open');settlementDemo?.setAttribute('aria-hidden','false')}
    function closeSettlementDemo(){settlementDemo?.classList.remove('open','victory');settlementDemo?.setAttribute('aria-hidden','true')}
    ...
    root.getElementById('show-victory-demo')?.addEventListener('click',()=>{settlementDemo?.classList.add('victory');window.setTimeout(()=>victoryExit?.focus(),420)});
