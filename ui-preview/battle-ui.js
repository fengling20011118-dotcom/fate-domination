...
function render(host,options={}){
 ...
 root.innerHTML='<style>'+css+'</style>'+markup;
  root.querySelector('style').textContent+=`
    ...
    .actions{max-width:430px}.actions .turn-clock{height:38px;display:grid;grid-template-columns:1fr 1fr;overflow:hidden;border:1px solid rgba(214,174,82,.36);border-radius:5px;background:linear-gradient(180deg,rgba(25,29,38,.98),rgba(10,13,19,.98));box-shadow:0 5px 14px rgba(0,0,0,.3)}
    .turn-clock>span{display:flex;align-items:center;justify-content:center;gap:10px;padding:0 12px;border-right:1px solid rgba(255,255,255,.08)}.turn-clock>span:last-child{border-right:0}.turn-clock small{color:#8c95a3;font-size:10px}.turn-clock strong{color:#f3e4b4;font:700 16px/1 Georgia,serif;letter-spacing:.04em}.turn-clock.overtime{border-color:rgba(198,74,81,.72);box-shadow:0 0 16px rgba(189,75,82,.18)}.turn-clock.overtime #round-time{color:#ff9da3}.turn-clock.bank-empty #bank-time{color:#ff737b}
    .play-action-row{grid-template-columns:96px minmax(0,1fr) minmax(0,1fr)}.play-action-row .end-action{height:38px}.turn-order{right:340px}.power-meter{cursor:help}.power-meter.projected{border-color:rgba(100,184,226,.72);box-shadow:0 0 12px rgba(92,169,214,.15)}.power-meter.projected strong{color:#aee0ff}
    .card.skill-staged{border-color:#d6ae52;box-shadow:0 9px 22px rgba(0,0,0,.7),0 0 18px rgba(214,174,82,.2)}.card.skill-staged .tag{color:#ffe29a}.card.skill-staged.selected::before{content:"再次点击取消";position:absolute;z-index:4;left:4px;top:5px;padding:3px 5px;border-radius:3px;background:rgba(8,10,15,.88);color:#f5df9c;font-size:8px}
    ...
    @media(max-width:1700px){.top-tools>button{height:32px}.turn-order{right:auto}.actions{max-width:390px}.turn-clock strong{font-size:14px}}
    @media(max-width:1260px){.top-tools{right:116px}.top-tools button{padding:0 8px}.actions{max-width:360px}.play-action-row{grid-template-columns:84px 1fr 1fr}}
  `;
 ...
 const api={root};
    ...
    root.querySelector('.hand').addEventListener('click',event=>{
      ...
      if(card.classList.contains('skill-staged')){
        ...
        toast.textContent='已取消使用【'+skillName+'】';toast.classList.add('show');setTimeout(()=>toast.classList.remove('show'),1500);
        return;
      }
    ...
    function isPlayableSkillCard(skill){return !!skill?.image&&skill.basePower!==null&&skill.basePower!==undefined}
    ...
    function stageSkillCard(skill,kind){
      const existing=stagedSkillCard(skill.name);
      if(existing){
        existing.remove();updateHandLayout();refreshPlaySelection();updateSkillButtonStates();
        toast.textContent='已取消使用【'+skill.name+'】';toast.classList.add('show');setTimeout(()=>toast.classList.remove('show'),1500);
        return;
      }
      if(handCards().filter(card=>card.classList.contains('selected')).length>=2){
        toast.textContent='出牌区已选满，请先取消一张牌';toast.classList.add('show');setTimeout(()=>toast.classList.remove('show'),1700);return;
      }
      const template=document.createElement('template');
      template.innerHTML=handCardHtml({...skill,id:'skill.'+kind+'.'+skill.name,attributes:['技能牌']});
      const card=template.content.firstElementChild;
      card.classList.add('skill-staged','selected');card.dataset.skillName=skill.name;card.dataset.skillKind=kind;
      ...
      toast.textContent='【'+skill.name+'】已加入出牌区，再次点击可取消';toast.classList.add('show');setTimeout(()=>toast.classList.remove('show'),1900);
    }
    function resolveConfirmedSkillCard(skillName){
      if(skillName!=='无限剑制')return;
      const targetHand=expandDeckCards(servantData.deck).slice(0,12),missing=targetHand.slice(handCards().length);
      drawCards(missing).then(()=>{toast.textContent='【无限剑制】已结算，手牌重组为 '+handCards().length+' 张';toast.classList.add('show');setTimeout(()=>toast.classList.remove('show'),1900)});
    }
    function executeImmediateSkill(skill){
      closeAnimatedModal(abilityModal,abilityPanel);
      if(skill.name==='固有时制御'){
        drawCards(1).then(()=>{toast.textContent='【固有时制御】抽取了 1 张牌';toast.classList.add('show');setTimeout(()=>toast.classList.remove('show'),1700)});return;
      }
      toast.textContent='已使用【'+skill.name+'】，等待规则结算';
      toast.classList.add('show');setTimeout(()=>toast.classList.remove('show'),1700);
    }
    root.querySelectorAll('[data-normal-skill]').forEach(button=>button.addEventListener('click',()=>{
      ...
      if(isPlayableSkillCard(skill)){stageSkillCard(skill,kind);return}
