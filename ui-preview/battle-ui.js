...
function render(host,options={}){
 ...
 root.querySelector('style').textContent+=`
  ...
  root.querySelector('style').textContent+=`
    ...
    .play-action-row { display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:6px; }
    ...
    @media(max-width:900px){.discard-columns{grid-template-columns:1fr}.discard-panel{height:min(760px,90vh)}.discard-pile{width:48px;height:68px}}
    ...
    .play-action-row{grid-template-columns:repeat(3,minmax(0,1fr))}.play-action-row .end-action{height:38px}.turn-order{right:340px}
    ...
    .play-action-row{grid-template-columns:repeat(3,minmax(0,1fr))}
    ...
    #confirm-hidden-play:not(:disabled){border-color:rgba(103,157,191,.82);color:#c8ebff;background:linear-gradient(180deg,rgba(22,43,58,.98),rgba(10,22,32,.98));box-shadow:0 0 0 1px rgba(104,190,236,.08),0 6px 18px rgba(0,0,0,.32)}
    ...
    .opp-panel-back .opp-skill-card,.opp-panel-back .mini-card{width:44px;height:61px}.opp-panel-back .opp-status-chip{max-width:170px;padding:5px 9px;font-size:11px!important}.opp-panel-back .opp-skill-card.revealed::after,.opp-panel-back .mini-card.played::after{padding:3px 1px;font-size:9px;line-height:12px;font-weight:800;letter-spacing:.04em}.opp-panel-back .opp-skill-card.revealed::after{color:#fff2c0;background:rgba(126,91,18,.94)}.opp-panel-back .mini-card.played::after{color:#ffe1e3;background:rgba(143,31,42,.94)}
    .status{display:none}
    .board{left:10px;right:10px;top:272px;bottom:300px;width:auto;height:auto;transform:none;grid-template-columns:repeat(6,minmax(0,1fr));grid-template-rows:minmax(0,1fr);grid-template-areas:"situation event workshop mountain city special";gap:6px}.board>.situation{grid-area:situation}.board>.event{grid-area:event}.board>.workshop{grid-area:workshop}.board>.mountain{grid-area:mountain}.board>.city{grid-area:city}.board>.special-locations{grid-area:special;min-width:0;min-height:0;display:grid;grid-template-rows:repeat(2,minmax(0,1fr));gap:6px}.special-locations>.place{min-width:0;min-height:0}.moon-cell{isolation:isolate;background-image:radial-gradient(circle at 50% 39%,rgba(199,214,255,.27) 0 7%,rgba(101,132,202,.18) 8% 18%,transparent 19%),repeating-radial-gradient(circle at 50% 39%,transparent 0 29px,rgba(177,196,245,.13) 30px 31px),linear-gradient(145deg,#222d4a,#080b16 68%)}.moon-cell::after{content:"";position:absolute;z-index:0;left:50%;top:39%;width:clamp(72px,7vw,132px);aspect-ratio:1;transform:translate(-50%,-50%);border:1px solid rgba(208,220,255,.4);border-radius:50%;box-shadow:0 0 28px rgba(117,151,226,.22),inset 0 0 24px rgba(117,151,226,.12)}.moon-cell-state{position:absolute;z-index:2;right:10px;top:10px;padding:4px 8px;border:1px solid rgba(186,203,244,.4);border-radius:3px;color:#d8e2ff;background:rgba(15,21,38,.82);font-size:9px;font-weight:800;letter-spacing:.12em}.moon-cell.expanded{border-color:rgba(126,190,255,.72);box-shadow:0 0 20px rgba(70,132,214,.16),0 8px 20px rgba(0,0,0,.48)}.moon-cell.expanded .moon-cell-state{color:#b9f2ff;border-color:rgba(83,206,239,.55);background:rgba(5,51,76,.82)}.moon-event-card{position:absolute;z-index:3;left:51px;top:8px;width:42px;height:60px;display:grid;place-items:end center;padding:4px 2px;border:1px solid rgba(143,196,236,.8);background:radial-gradient(circle at 50% 32%,rgba(56,216,255,.48),transparent 28%),linear-gradient(145deg,#233c67,#07111f);box-shadow:0 5px 12px rgba(0,0,0,.58);color:#dff8ff;font-size:7px;font-weight:800;cursor:help}.moon-event-card::before{content:"◇";position:absolute;left:50%;top:43%;transform:translate(-50%,-50%);color:#9beaff;font-size:22px;text-shadow:0 0 10px rgba(80,218,255,.7)}.moon-event-card span{position:relative;z-index:1}.moon-rule-chip{position:absolute;z-index:3;left:101px;top:10px;max-width:105px;padding:4px 6px;border-left:2px solid #5fd6ef;color:#d7edff;background:rgba(5,16,30,.72);font-size:8px;font-weight:700;line-height:1.25;cursor:help}.moon-mana-slots{position:absolute;z-index:3;right:8px;bottom:8px;display:flex;gap:5px}.moon-mana-slots>span{display:grid;justify-items:center;gap:2px}.moon-mana-slots i{width:25px;height:25px;border:2px solid #b8d7ee;border-radius:50%;background:linear-gradient(145deg,rgba(31,76,113,.72),rgba(5,17,31,.92));box-shadow:0 0 10px rgba(85,193,242,.24)}.moon-mana-slots b{color:#d9f3ff;font-size:7px;white-space:nowrap;text-shadow:0 2px 4px #000}.moon-cell .place-title{max-width:44%;right:auto}.moon-cell .base-vp{border-color:#7fdcff;color:#c8f5ff}.moon-cell .base-vp::after{content:"战果"}@media(max-width:1500px){.moon-rule-chip{display:none}.moon-event-card{left:48px;width:36px;height:51px}.moon-mana-slots i{width:21px;height:21px}.moon-mana-slots b{font-size:6px}.moon-cell-state{padding:3px 5px;font-size:8px}}.special-locations .place-title{font-size:clamp(14px,.9vw,17px)}.special-locations .base-vp{left:10px;top:8px;width:34px;height:34px;min-width:34px;font-size:15px}.special-locations .base-vp::after{top:34px;font-size:8px}
    .moon-cell{background-image:linear-gradient(180deg,rgba(3,13,31,.18),rgba(2,8,19,.78)),url("../assets/map/locations/moon-cell-back.png");background-position:center 46%;background-size:cover}.special-locations .moon-cell .place-title{max-width:48%;font-size:20px;line-height:1.05}.special-locations .moon-cell .place-title small{margin-top:5px;font-size:11px;line-height:1.2}.moon-cell .base-vp{width:38px;height:38px;min-width:38px;font-size:17px}.moon-cell .base-vp::after{top:38px;font-size:9px}.moon-event-card{left:57px;width:48px;height:68px;font-size:10px}.moon-event-card::before{font-size:27px}.moon-cell-state{padding:5px 8px;font-size:10px}.moon-rule-chip{left:114px;max-width:118px;padding:5px 7px;font-size:10px}.moon-mana-slots{gap:7px}.moon-mana-slots i{width:34px;height:34px}.moon-mana-slots b{font-size:10px;line-height:1.1}@media(max-width:1500px){.special-locations .moon-cell .place-title{left:95px;right:8px;bottom:48px;max-width:none;font-size:18px;white-space:nowrap}.special-locations .moon-cell .place-title small{display:none}.moon-event-card{left:49px;width:38px;height:54px;font-size:9px}.moon-cell-state{padding:4px 6px;font-size:9px}.moon-mana-slots{gap:5px}.moon-mana-slots i{width:27px;height:27px}.moon-mana-slots b{font-size:9px}}
    .deck-discard-zone{position:absolute;z-index:5;right:12%;top:50%;transform:translateY(-50%)}.situation>.deck-display,.event>.deck-display{left:38%}.discard-pile.deck-discard-pile{width:clamp(48px,3.15vw,61px);height:auto;aspect-ratio:.7}.discard-pile.deck-discard-pile .discard-label{width:76px}.discard-columns.single-kind{grid-template-columns:minmax(0,1fr)}.discard-columns.single-kind .discard-column{width:100%}
    ...
    .self-player .public-cards{height:44px;padding:5px 9px}.self-player .opp-detail-content{padding:24px 18px 16px;grid-template-columns:1.55fr 1fr;grid-template-rows:minmax(0,1fr) auto;gap:16px 20px}.self-player .opp-detail-label{margin-bottom:10px;font-size:14px}.self-player .opp-skill-strip,.self-player .opp-played-strip{gap:10px}.self-player .opp-panel-back .opp-skill-card,.self-player .opp-panel-back .mini-card{width:76px;height:106px}.self-player .opp-panel-back .opp-skill-card.revealed::after,.self-player .opp-panel-back .mini-card.played::after{padding:5px 2px;font-size:12px;line-height:17px;font-weight:900;letter-spacing:.06em}.self-player .opp-panel-back .opp-card-overflow{width:68px;height:106px;font-size:15px}.self-player .opp-detail-group.status-group{grid-template-columns:52px minmax(0,1fr)}.self-player .opp-status-chip{max-width:230px;padding:7px 11px;font-size:13px!important}.self-player .opp-panel-back .opp-detail-head{right:8px;top:8px;width:30px;height:30px}.self-player .opp-panel-back .opp-detail-head .opp-panel-toggle{width:30px;height:30px;min-width:30px}
    ...
    .play-action-row{grid-template-columns:repeat(3,minmax(0,1fr));gap:7px}
    ...
    /* Played cards lie directly on the battlefield art, grouped by current location. */
    .map-played-layer{position:absolute;z-index:8;left:7px;right:7px;top:13px;display:flex;justify-content:flex-end;align-items:flex-start;gap:9px;pointer-events:none;perspective:700px}
    .table-play-seat{position:relative;flex:0 0 76px;height:72px;display:block;pointer-events:none;filter:drop-shadow(0 7px 7px rgba(0,0,0,.72));transform:rotate(-1deg)}
    .table-play-seat:nth-child(even){transform:translateY(3px) rotate(1.2deg)}
    .table-play-owner{position:absolute;z-index:5;left:-5px;bottom:-4px;width:24px;height:24px;display:block;border:2px solid rgba(230,235,242,.9);border-radius:50%;background:#090d14;box-shadow:0 3px 8px rgba(0,0,0,.8);overflow:hidden;pointer-events:auto}
    .table-play-owner img{width:100%;height:100%;display:block;object-fit:cover}
    .table-play-owner span{position:absolute;width:1px;height:1px;overflow:hidden;clip-path:inset(50%)}
    .table-play-seat.is-self .table-play-owner{width:27px;height:27px;border-color:var(--gold);box-shadow:0 3px 9px rgba(0,0,0,.82),0 0 9px rgba(214,174,82,.55)}
    .table-play-label{display:none}
    .table-play-cards{position:relative;height:68px;display:flex;align-items:flex-start;padding-left:2px;overflow:visible;isolation:isolate}
    .table-play-cards .mini-card{position:relative;width:46px;height:65px;flex:0 0 46px;pointer-events:auto;border:1px solid rgba(215,222,232,.82);border-radius:2px;box-shadow:0 8px 14px rgba(0,0,0,.82);transform:rotate(-4deg);transition:transform .16s ease,filter .16s ease}
    .table-play-cards .mini-card+.mini-card{margin-left:-17px;transform:translateY(3px) rotate(4deg)}
    .table-play-cards .mini-card:hover{z-index:8;transform:translateY(-8px) rotate(0deg) scale(1.14);filter:brightness(1.08)}
    .table-play-seat.is-self .table-play-cards .mini-card{width:50px;height:71px;flex-basis:50px;border:2px solid var(--gold);box-shadow:0 9px 17px rgba(0,0,0,.84),0 0 12px rgba(214,174,82,.3)}
    .table-play-cards .mini-card.played::after{display:none}
    .table-play-cards .mini-card.facedown-play{border-color:#73b8de;background:#0c1722;box-shadow:0 8px 14px rgba(0,0,0,.82),0 0 10px rgba(83,170,220,.28)}
    .table-play-cards .mini-card.facedown-play .deck-back{width:100%;height:100%;display:block;color:transparent;background:#0b1019 url("../assets/cards/deck/attack-back.png") center/cover no-repeat}
    .table-card-flight.facedown .flight-card-back{width:100%;height:100%;display:block;background:#0b1019 url("../assets/cards/deck/attack-back.png") center/cover no-repeat}
    .table-play-cards .opp-card-overflow{position:absolute;z-index:9;right:-4px;top:-7px;width:23px;height:23px;display:grid;place-items:center;border:1px solid #ff8e96;border-radius:50%;color:#fff;background:#9d3039;font:900 10px/1 Bahnschrift,"Segoe UI",sans-serif;box-shadow:0 3px 8px rgba(0,0,0,.75);pointer-events:auto}
    .table-play-empty{display:none}
    .mountain .map-played-layer,.city .map-played-layer{top:78px}
    .workshop .map-played-layer{top:86px}
    .special-locations .map-played-layer{top:7px;right:6px;gap:4px}
    .special-locations .table-play-seat{flex-basis:55px;height:52px}
    .special-locations .table-play-owner{width:19px;height:19px}
    .special-locations .table-play-cards .mini-card{width:34px;height:48px;flex-basis:34px}
    @media(max-width:1500px){.map-played-layer{gap:5px}.table-play-seat{flex-basis:65px;height:62px}.table-play-owner{width:21px;height:21px}.table-play-seat.is-self .table-play-owner{width:23px;height:23px}.table-play-cards{height:58px}.table-play-cards .mini-card{width:39px;height:55px;flex-basis:39px}.table-play-cards .mini-card+.mini-card{margin-left:-15px}.table-play-seat.is-self .table-play-cards .mini-card{width:43px;height:61px;flex-basis:43px}.table-play-cards .opp-card-overflow{width:20px;height:20px;font-size:9px}.mountain .map-played-layer,.city .map-played-layer{top:68px}.workshop .map-played-layer{top:75px}.opp-panel-back .opp-skill-card{width:58px;height:81px}}
    @media(max-height:760px){.map-played-layer{top:7px;gap:4px}.table-play-seat{flex-basis:56px;height:50px}.table-play-owner{width:18px;height:18px}.table-play-cards{height:48px}.table-play-cards .mini-card,.table-play-seat.is-self .table-play-cards .mini-card{width:32px;height:45px;flex-basis:32px}.table-play-cards .mini-card+.mini-card{margin-left:-12px}.mountain .map-played-layer,.city .map-played-layer,.workshop .map-played-layer{top:50px}}
    .table-card-flight{position:fixed;z-index:95;overflow:hidden;border:2px solid var(--gold);background:#0b0f16;box-shadow:0 16px 38px rgba(0,0,0,.76),0 0 20px rgba(214,174,82,.25);pointer-events:none;will-change:transform,opacity}.table-card-flight img{width:100%;height:100%;display:block;object-fit:contain}
    .opp-detail-content{grid-template-columns:1fr;grid-template-rows:minmax(0,1fr) auto}.opp-detail-group.status-group{grid-column:1}.opp-skill-strip{gap:10px;align-items:flex-start}.opp-panel-back .opp-skill-card{width:74px;height:104px}
    .self-player .opp-detail-content{grid-template-columns:1fr;grid-template-rows:minmax(0,1fr) auto}.self-player .opp-skill-strip{gap:14px}.self-player .opp-panel-back .opp-skill-card{width:88px;height:123px}
    .opp-panel-toggle{width:24px!important;height:24px!important;min-width:24px!important;padding:0!important;border:1px solid rgba(214,174,82,.58)!important;border-radius:3px!important;color:#ead18a!important;background:linear-gradient(180deg,rgba(39,35,26,.98),rgba(17,18,21,.98))!important;font:900 11px/1 "Microsoft YaHei","Segoe UI",sans-serif!important;letter-spacing:0;box-shadow:0 3px 8px rgba(0,0,0,.34)!important}
    .opp-panel-toggle span{display:block;font:17px/1 Georgia,serif}.opp-panel-toggle::after{content:none!important}
    .opp-panel-front .opp-info>b{padding-right:27px;font-size:13px}.opp-panel-front .opp-panel-toggle{z-index:6;left:auto!important;right:5px!important;top:5px!important}
    .opp-panel-toggle:not(:disabled):hover{transform:translateY(-1px)!important;border-color:#efd47e!important;color:#fff2bd!important;background:linear-gradient(180deg,#4a4027,#242119)!important;box-shadow:0 5px 12px rgba(0,0,0,.45),0 0 10px rgba(214,174,82,.2)!important}
    .opp-panel-toggle:not(:disabled):active{transform:translateY(1px)!important;box-shadow:0 1px 4px rgba(0,0,0,.48) inset!important}
    .opp-panel-back .opp-detail-head{right:5px;top:5px;width:24px;height:24px}.opp-panel-back .opp-detail-head .opp-panel-toggle{inset:0!important;width:24px!important;height:24px!important;min-width:24px!important}
    .self-player .opp-panel-front .opp-info>b{padding-right:38px}.self-player .opp-panel-front .opp-panel-toggle{left:auto!important;right:8px!important;top:8px!important}.self-player .opp-panel-toggle{width:30px!important;height:30px!important;min-width:30px!important}.self-player .opp-panel-back .opp-detail-head{right:8px;top:8px;width:30px;height:30px}.self-player .opp-panel-back .opp-detail-head .opp-panel-toggle{width:30px!important;height:30px!important;min-width:30px!important}
    .board{grid-template-columns:minmax(190px,.66fr) minmax(0,1.16fr) minmax(0,1.42fr) minmax(0,1.16fr) minmax(150px,.62fr) minmax(0,1.16fr);grid-template-rows:repeat(2,minmax(0,1fr));grid-template-areas:"situation workshop mountain city scout moon" "event workshop mountain city scout moon"}
    .board>.situation{grid-area:situation}.board>.event{grid-area:event}.board>.workshop{grid-area:workshop}.board>.mountain{grid-area:mountain}.board>.city{grid-area:city}
    .board>.special-locations{display:contents}.special-locations>.moon-cell{grid-area:moon}.special-locations>.scout{grid-area:scout}
    .situation>.deck-display,.event>.deck-display{left:48%;top:47%;width:40px;transform:translate(-50%,-50%)}
    .situation .deck-discard-zone,.event .deck-discard-zone{right:9%;top:46%}.situation .discard-pile.deck-discard-pile,.event .discard-pile.deck-discard-pile{width:34px}.situation .discard-pile.deck-discard-pile .discard-label,.event .discard-pile.deck-discard-pile .discard-label{display:none}
    .situation .place-title,.event .place-title{left:9px;right:auto;bottom:7px;padding:0;border:0;font-size:15px}.situation .place-title small,.event .place-title small{font-size:8px;margin-top:1px}
    .special-locations>.place{min-width:0;min-height:0}.special-locations .place-title{font-size:clamp(17px,1.05vw,20px)}.special-locations .base-vp{left:12px;top:10px;width:42px;height:42px;min-width:42px;font-size:18px}.special-locations .base-vp::after{top:42px;font-size:9px}
 ...
 const api={root};
    ...
    function buildBattleRoster(){
      ...
      const locations=['深山町','新都','深山町','魔术工坊','深山町','侦察'];
      const current={master:masterData,servant:servantData,mana:7,seals:3,location:'深山町',power:5,vp:6,handCount:4,deckCount:8,discardCount:0,activeAttackCount:0,current:true,nameRevealed:true};
    ...
    function tablePlayedCards(player,rosterIndex){
      if(Array.isArray(player?.activeCards))return player.activeCards.filter(Boolean);
      const deck=(player?.servant?.deck||[]).filter(Boolean),count=Math.max(0,Number(player?.activeAttackCount||0));
      return Array.from({length:count},(_,i)=>deck[((Math.max(0,rosterIndex-1)*2+i)%Math.max(1,deck.length))]).filter(Boolean);
    }
    function tablePlayedCardsHtml(player,rosterIndex){
      const cards=tablePlayedCards(player,rosterIndex),visible=cards.slice(0,2);
      if(!cards.length)return '<span class="table-play-empty">尚未出牌</span>';
      const faces=visible.map(card=>card.faceDown?'<span class="mini-card played facedown-play" data-info="暗置牌|牌面尚未公开。"><span class="deck-back">◇</span></span>':'<span class="mini-card played" data-info="'+h(card.name||'已打出的牌')+'|'+h((card.type||'攻击')+' · 威力 '+(card.basePower??0)+'\n'+(card.text||''))+'">'+(card.image?'<img src="'+h(card.image)+'" alt="'+h(card.name||'已打出的牌')+'">':'<span class="deck-back">'+h(card.basePower??'?')+'</span>')+'</span>').join('');
      const hidden=cards.slice(visible.length),stack=hidden.map(card=>'<i hidden data-stack-src="'+h(card.faceDown?'':(card.image||''))+'" data-stack-name="'+h(card.faceDown?'暗置牌':(card.name||'已打出的牌'))+'"></i>').join('');
      return faces+(hidden.length?'<span class="opp-card-overflow" data-info="桌面上的出牌|另有 '+hidden.length+' 张已打出的牌">'+stack+'+'+hidden.length+'</span>':'');
    }
    function battlefieldForPlayer(player){
      const selector={深山町:'.mountain',新都:'.city',侦察:'.scout',魔术工坊:'.workshop',月之圣杯:'.moon-cell'}[player?.location];
      return selector?root.querySelector(selector):null;
    }
    function renderTablePlayedCards(){
      root.querySelectorAll('.map-played-layer').forEach(layer=>layer.remove());
      const layers=new Map();
      battleRoster.forEach((player,index)=>{
        if(!tablePlayedCards(player,index).length)return;
        const battlefield=battlefieldForPlayer(player);if(!battlefield)return;
        let layer=layers.get(battlefield);
        if(!layer){layer=document.createElement('div');layer.className='map-played-layer';layer.setAttribute('aria-label','战场上的已打出卡牌');battlefield.append(layer);layers.set(battlefield,layer)}
        const seat=document.createElement('article');seat.className='table-play-seat '+(index===0?'is-self':'');seat.dataset.tableRoster=String(index);
        seat.innerHTML='<header class="table-play-owner" title="'+h(player.master.name)+(index===0?' · 你':'')+'"><img src="'+h(masterImg(player.master))+'" alt=""><span>'+h(player.master.name)+(index===0?' · 你':'')+'</span></header><div class="table-play-cards">'+tablePlayedCardsHtml(player,index)+'</div>';
        layer.append(seat);
      });
    }
    function playedCardFromElement(card){
      const raw=String(card.dataset.info||''),parts=raw.split('|'),detail=parts.slice(1).join('|');
      return {name:parts[0]||card.querySelector('.tag')?.textContent?.trim()||'攻击牌',image:card.querySelector('img')?.getAttribute('src')||'',type:card.dataset.traits||'攻击',basePower:Number(card.dataset.power)||0,text:detail};
    }
    function animateCardToPublicTable(source,target,faceDown=false){
      if(!source||!target)return Promise.resolve();
      const from=source.getBoundingClientRect(),to=target.getBoundingClientRect(),reduced=window.matchMedia&&window.matchMedia('(prefers-reduced-motion: reduce)').matches;
      if(reduced||!from.width||!to.width)return Promise.resolve();
      const flight=document.createElement('div');flight.className='table-card-flight'+(faceDown?' facedown':'');flight.style.cssText='left:'+from.left+'px;top:'+from.top+'px;width:'+from.width+'px;height:'+from.height+'px';flight.innerHTML=faceDown?'<span class="flight-card-back"></span>':(source.querySelector('img')?.outerHTML||'');root.append(flight);
      return new Promise(resolve=>{const dx=to.left-from.left,dy=to.top-from.top,sx=to.width/from.width,sy=to.height/from.height,animation=flight.animate([{transform:'translate3d(0,0,0) rotateZ(0deg)',opacity:1},{offset:.56,transform:'translate3d('+(dx*.7)+'px,'+(dy*.62)+'px,90px) rotateZ(-4deg) scale(.82)',opacity:1},{transform:'translate3d('+dx+'px,'+dy+'px,0) scale('+sx+','+sy+')',opacity:1}],{duration:560,easing:'cubic-bezier(.18,.78,.2,1)',fill:'forwards'});animation.onfinish=()=>{flight.remove();resolve()};animation.oncancel=animation.onfinish});
    }
    ...
    function opponentBackHtml(player,opponentIndex,revealed=player.nameRevealed!==false){
      return '<div class="opp-detail-content"><section class="opp-detail-group"><span class="opp-detail-label">从者技能</span><span class="opp-skill-strip">'+[0,1,2].map(slot=>skillItemHtml(player,slot,revealed)).join('')+'</span></section><section class="opp-detail-group status-group"><span class="opp-detail-label">状态</span><span class="opp-status-row">'+opponentStateHtml(player,opponentIndex,revealed)+'</span></section></div>';
    }
    root.addEventListener('click',event=>{
    ...
    function renderCardPreview(target,event,epoch=previewEpoch){
      ...
      if(opponent&&target.matches('.opp-panel-front > img')){
        ...
        const stack=[...target.querySelectorAll('[data-stack-src]')];title='公共桌面';detail='另有 '+stack.length+' 张已打出的牌';img='';
    ...
    const confirmPlay = root.getElementById('confirm-play'),confirmHiddenPlay=root.getElementById('confirm-hidden-play'),handPowerNode=root.getElementById('hand-power-value');
    let toastTimer=0;
    function hideToast(){
    ...
    function refreshPlaySelection() {
      ...
      confirmPlay.textContent='出牌 '+selected.length+'/2';
      confirmHiddenPlay.textContent=selected.length&&selected.every(card=>card.classList.contains('play-facedown'))?'改为明置':'暗置所选';
      confirmPlay.disabled=selected.length!==2;
      confirmHiddenPlay.disabled=selected.length===0;
    ...
    root.querySelector('.hand').addEventListener('click',event=>{
      ...
      if(!willSelect)card.classList.remove('play-facedown');
      playSfx(willSelect?'card-select':'card-deselect',{volume:.1});
      refreshPlaySelection();
    });
    function submitSelectedCards(){
      const selected=handCards().filter(card=>card.classList.contains('selected'));
      if(selected.length!==2)return;
      const faceDownCards=selected.filter(card=>card.classList.contains('play-facedown'));
      const faceUpCards=selected.filter(card=>!card.classList.contains('play-facedown'));
      const confirmedSkills=faceUpCards.filter(card=>card.dataset.skillName).map(card=>card.dataset.skillName);
      playSfx('card-play',{volume:.3});
      const played=selected.map(card=>({...playedCardFromElement(card),faceDown:card.classList.contains('play-facedown')}));
      selected.forEach(card=>card.classList.remove('selected'));
      battleRoster[0].activeCards=played;
      battleRoster[0].activeAttackCount=played.length;
      renderTablePlayedCards();
      const targets=[...tablePlayZone.querySelectorAll('[data-table-roster="0"] .mini-card')];
      const flights=selected.map((card,index)=>animateCardToPublicTable(card,targets[index],played[index].faceDown));
      confirmedSkills.forEach(resolveConfirmedSkillCard);
      if(typeof options.onPlayCards==='function')options.onPlayCards({cards:played,faceUpCards:played.filter(card=>!card.faceDown),faceDownCards:played.filter(card=>card.faceDown),player:battleRoster[0]});
      ...
      showToast('已出牌 · 明置 '+faceUpCards.length+' · 暗置 '+faceDownCards.length,1500);
    }
    confirmPlay.addEventListener('click',submitSelectedCards);
    confirmHiddenPlay.addEventListener('click',()=>{
      const selected=handCards().filter(card=>card.classList.contains('selected'));
      if(!selected.length)return;
      const makeFaceDown=selected.some(card=>!card.classList.contains('play-facedown'));
      selected.forEach(card=>card.classList.toggle('play-facedown',makeFaceDown));
      playSfx('ui-tab',{volume:.5});
      refreshPlaySelection();
      showToast(makeFaceDown?'已将所选卡牌设为暗置':'已将所选卡牌改为明置',1200);
    });
    ...
    const phaseNodes=[...root.querySelectorAll('.phase [data-phase]')],phaseNames={prepare:'准备阶段',outpost:'部署阶段',action:'行动阶段',battle:'战斗阶段',settlement:'结算阶段'},phaseClockSeconds={prepare:60,outpost:120,action:120,battle:60},timedPhases=new Set(Object.keys(phaseClockSeconds));
    const defaultBankSeconds=1200,operationSeconds=120,playerTimeBanks=new Map(battleRoster.map((player,index)=>[index,Math.max(0,Math.floor(Number(player.bankRemaining??defaultBankSeconds)||0))]));
    const turnClockState={phase:'action',running:true,roundRemaining:120,bankRemaining:playerTimeBanks.get(0),overtime:false,autoPlayed:false,clockOwnerIndex:0,clockOwnerName:battleRoster[0]?.master?.name||'当前玩家',interaction:false,bankExhausted:false},interactionClockStack=[];
    ...
    function renderTurnClock(){
      ...
      if(phaseTimeLabel)phaseTimeLabel.textContent=!turnClockState.running?'不计时':turnClockState.interaction?(turnClockState.clockOwnerName+'回应'):(phaseNames[turnClockState.phase]||'本阶段');
      ...
      if(turnClock){const owner=turnClockState.clockOwnerName;if(!turnClockState.running)turnClock.title=(phaseNames[turnClockState.phase]||'当前阶段')+'不计时';else if(turnClockState.overtime)turnClock.title=owner+'的阶段时间已用完，正在消耗个人总时长';else if(turnClockState.bankExhausted)turnClock.title=owner+'的个人总时长已耗尽，本次操作剩余 '+formatClock(turnClockState.roundRemaining)+' 后自动处理';else turnClock.title='准备、战斗阶段各 1 分钟；部署、行动阶段各 2 分钟；超时后消耗 '+owner+' 的个人总时长'}
    }
    function resolveClockOwner(ref){if(ref==null)return turnClockState.clockOwnerIndex;if(Number.isInteger(ref)&&battleRoster[ref])return ref;if(typeof ref==='string'){const number=Number(ref);if(Number.isInteger(number)&&battleRoster[number])return number;const index=battleRoster.findIndex(player=>player?.master?.name===ref||player?.master?.fullName===ref);if(index>=0)return index}if(typeof ref==='object'){if(Number.isInteger(ref.rosterIndex)&&battleRoster[ref.rosterIndex])return ref.rosterIndex;return resolveClockOwner(ref.master?.name||ref.name)}return -1}
    function saveActiveBank(){playerTimeBanks.set(turnClockState.clockOwnerIndex,Math.max(0,turnClockState.bankRemaining))}
    function phaseWindowSeconds(phase=turnClockState.phase){return phaseClockSeconds[phase]||0}
    function resetPhaseWindow(){turnClockState.bankExhausted=turnClockState.bankRemaining<=0;turnClockState.roundRemaining=turnClockState.interaction?operationSeconds:phaseWindowSeconds();turnClockState.overtime=false;turnClockState.autoPlayed=false}
    function setClockOwner(ref,{interaction=false,reset=true}={}){const index=resolveClockOwner(ref);if(index<0)return false;saveActiveBank();turnClockState.clockOwnerIndex=index;turnClockState.clockOwnerName=battleRoster[index]?.master?.name||('玩家 '+(index+1));turnClockState.bankRemaining=playerTimeBanks.get(index)??defaultBankSeconds;turnClockState.interaction=Boolean(interaction);if(reset)resetPhaseWindow();renderTurnClock();return true}
    function beginInteractionClock(ref){const index=resolveClockOwner(ref);if(index<0)return false;interactionClockStack.push({...turnClockState});return setClockOwner(index,{interaction:true,reset:true})}
    function endInteractionClock(){const previous=interactionClockStack.pop();if(!previous)return false;saveActiveBank();Object.assign(turnClockState,previous,{bankRemaining:playerTimeBanks.get(previous.clockOwnerIndex)??previous.bankRemaining});renderTurnClock();return true}
    function setBattlePhase(phase,{reset=true}={}){
      ...
      turnClockState.interaction=false;
      interactionClockStack.length=0;
      turnClockState.overtime=false;
      turnClockState.autoPlayed=false;
      if(turnClockState.running&&reset)resetPhaseWindow();
    ...
    function autoPlayOnTimeout(){
      ...
      if(turnClockState.phase==='outpost'){
        options.onAutoDeploy?.({player:battleRoster[turnClockState.clockOwnerIndex],playerIndex:turnClockState.clockOwnerIndex});
        showToast('部署计时结束，已自动部署至默认地点',2200);
        window.setTimeout(()=>setBattlePhase('action'),260);
        return true;
      }
      if(turnClockState.phase==='battle'){showToast('战斗阶段计时结束，已自动完成响应',2200);window.setTimeout(()=>setBattlePhase('settlement'),260);return true}
    ...
    function tickTurnClock(){
      ...
      if(turnClockState.roundRemaining>0){
        ...
        playerTimeBanks.set(turnClockState.clockOwnerIndex,turnClockState.bankRemaining);
        if(turnClockState.bankRemaining===0)autoPlayOnTimeout();
      }else autoPlayOnTimeout();
      renderTurnClock();
    }
    function setTurnClock(roundSeconds=phaseWindowSeconds(),bankSeconds=turnClockState.bankRemaining){
      ...
      playerTimeBanks.set(turnClockState.clockOwnerIndex,turnClockState.bankRemaining);
      turnClockState.bankExhausted=turnClockState.bankRemaining<=0;
      turnClockState.overtime=turnClockState.roundRemaining<=0&&turnClockState.bankRemaining>0;
      turnClockState.autoPlayed=false;
      renderTurnClock();
    }
    function resetTurnClock(){setTurnClock(turnClockState.interaction?operationSeconds:phaseWindowSeconds(),turnClockState.bankRemaining)}
    ...
    let activeChoiceSkill=null,activeChoiceText='',activeChoiceOwnsInteraction=false;
    function openChoice(skill,source,responder){const choiceItems=choiceLines(skill);if(!choiceItems.length)return false;if(responder!=null){activeChoiceOwnsInteraction=beginInteractionClock(responder);if(!activeChoiceOwnsInteraction)return false}activeChoiceSkill=skill;activeChoiceText='';choiceType.textContent=(source||'技能')+' · '+(skill.type||'');choiceTitle.textContent=skill.name;choiceDesc.textContent=String(skill.text||'').split(/\r?\n/).filter(x=>!/^\s*[-－•]/.test(x)).join(' ');choiceOptions.innerHTML=choiceItems.map((x,i)=>'<button class="choice-option" type="button" data-choice-value="'+h(x)+'" data-index="0'+(i+1)+'">'+h(x)+'</button>').join('');confirmChoice.disabled=true;openAnimatedModal(choiceModal,choiceModal.querySelector('.choice-panel'));const choiceMotion=motionApi(),choiceButtons=[...choiceOptions.querySelectorAll('.choice-option')];if(choiceMotion&&choiceButtons.length)choiceMotion.animate(choiceButtons,{opacity:[0,1],y:[12,0],delay:choiceMotion.stagger?choiceMotion.stagger(55):0,duration:260,ease:'outCubic'});return true}
    ...
    function closeChoice(){closeAnimatedModal(choiceModal,choiceModal.querySelector('.choice-panel'));activeChoiceSkill=null;activeChoiceText='';confirmChoice.disabled=true;if(activeChoiceOwnsInteraction){activeChoiceOwnsInteraction=false;endInteractionClock()}}
    ...
    function renderDiscardView(){
      ['situation','event'].forEach(kind=>{
        ...
        if(pile){
          pile.classList.toggle('empty',!cards.length);
          const count=pile.querySelector('.discard-count'),img=pile.querySelector('img'),label=kind==='situation'?'局势牌':'事件牌';
          if(count)count.textContent=String(cards.length);
          if(cards.length&&img){const top=cards[cards.length-1];img.src=top.image;img.alt=top.name+' · '+label+'弃牌堆顶牌'}
          else if(img){img.removeAttribute('src');img.alt=label+'弃牌堆'}
        }
      });
    }
    ...
    function commitRoundDiscards(){
      ...
      const targets={situation:root.querySelector('[data-discard-kind="situation"]'),event:root.querySelector('[data-discard-kind="event"]')};
      Object.values(targets).forEach(target=>{if(target)target.style.opacity='0'});
      renderDiscardView();
      return Promise.all(cards.map((card,index)=>animateResolvedDiscard(card,targets[card.kind],index*150))).then(()=>{
        Object.values(targets).forEach(target=>{if(target){target.style.removeProperty('opacity');target.classList.remove('landing');void target.offsetWidth;target.classList.add('landing')}});
        refreshPlaySelection();
    ...
    function openDiscardModal(kind='situation'){
      const activeKind=kind==='event'?'event':'situation',label=activeKind==='situation'?'局势牌':'事件牌',cards=publicDiscards[activeKind];
      hidePreview();renderDiscardView();
      root.querySelectorAll('[data-discard-column]').forEach(column=>column.hidden=column.dataset.discardColumn!==activeKind);
      const title=root.getElementById('discard-modal-title'),meta=root.getElementById('discard-modal-meta');
      if(title)title.textContent=label+'弃牌堆';
      if(meta)meta.textContent='结算后公开 · '+cards.length+' 张';
      openAnimatedModal(discardModal,discardPanel);discardModal?.setAttribute('aria-hidden','false')
    }
    ...
    root.querySelectorAll('.deck-discard-zone').forEach(zone=>zone.addEventListener('click',event=>{const pile=event.target.closest('.discard-pile:not(.empty)');if(pile)openDiscardModal(pile.dataset.discardKind)}));
    ...
    api.getPlayerTimeBanks=()=>battleRoster.map((player,index)=>({playerIndex:index,name:player.master?.name||('玩家 '+(index+1)),seconds:playerTimeBanks.get(index)??defaultBankSeconds}));
    api.setClockOwner=setClockOwner;
    api.beginInteractionClock=beginInteractionClock;
    api.endInteractionClock=endInteractionClock;
    api.openTimedChoice=(skill,source,responder)=>openChoice(skill,source,responder);
