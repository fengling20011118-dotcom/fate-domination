...
function render(host,options={}){
 ...
 root.innerHTML='<style>'+css+'</style>'+markup;
  root.querySelector('style').textContent+=`
    ...
    .play-action-row{grid-template-columns:minmax(0,1fr) minmax(0,1fr)}.play-action-row .end-action{height:38px}.turn-order{right:340px}.power-meter{cursor:help}.power-meter.projected{border-color:rgba(100,184,226,.72);box-shadow:0 0 12px rgba(92,169,214,.15)}.power-meter.projected strong{color:#aee0ff}
    ...
    @media(max-width:1260px){.top-tools{right:116px}.top-tools button{padding:0 8px}.move-action-row{grid-template-columns:1fr 88px}}
  `;
 ...
 const api={root};
    ...
    function skillInteractionMode(skill){
      const rules=(skill?.type||'')+'\n'+(skill?.text||'');
      if(/打出时\s*[：:]/.test(rules))return 'on-use';
      if(skill?.image&&/战斗阶段/.test(rules))return 'attack-card';
      return 'ability';
    }
    ...
    function executeImmediateSkill(skill){
      ...
      if(skill.name==='无限剑制'){
        const targetHand=expandDeckCards(servantData.deck).slice(0,12),missing=targetHand.slice(handCards().length);
        drawCards(missing).then(()=>{toast.textContent='【无限剑制】已立即生效，手牌重组为 '+handCards().length+' 张';toast.classList.add('show');setTimeout(()=>toast.classList.remove('show'),1900)});return;
      }
      if(skill.name==='固有时制御'){
    ...
    root.querySelectorAll('[data-normal-skill]').forEach(button=>button.addEventListener('click',()=>{
      ...
      const interaction=skillInteractionMode(skill);
      if(interaction==='attack-card'){stageSkillCard(skill,kind);return}
