...
function render(host,options={}){
 ...
 const api={root};
    ...
    function tablePlayedCardsHtml(player,rosterIndex){
      ...
      const faces=visible.map(card=>'<span class="mini-card played" data-info="'+h(card.name||'已打出的牌')+'|'+h((card.type||'攻击')+' · 威力 '+(card.basePower??0)+'\n'+(card.text||''))+'">'+(card.image?'<img src="'+h(card.image)+'" alt="'+h(card.name||'已打出的牌')+'">':'<span class="deck-back">'+h(card.basePower??'?')+'</span>')+'</span>').join('');
      const hidden=cards.slice(visible.length),stack=hidden.map(card=>'<i hidden data-stack-src="'+h(card.image||'')+'" data-stack-name="'+h(card.name||'已打出的牌')+'"></i>').join('');
    ...
    function animateCardToPublicTable(source,target){
      ...
      const flight=document.createElement('div');flight.className='table-card-flight';flight.style.cssText='left:'+from.left+'px;top:'+from.top+'px;width:'+from.width+'px;height:'+from.height+'px';flight.innerHTML=source.querySelector('img')?.outerHTML||'';root.append(flight);
    ...
    confirmPlay.addEventListener('click', () => {
      ...
      const flights=selected.map((card,index)=>animateCardToPublicTable(card,targets[index]));
